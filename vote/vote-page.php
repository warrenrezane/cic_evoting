...
<script type="text/javascript">
    ...
    function getCandidate() {
        $.ajax({
            ...
            data: {
                ...
                election_id: '<?php echo $latestElection; ?>'
            },
            success: (e) => {
        ...
        $.each(_candidate, (index, value) => {
            if (curPositionName != value.positionname) {
                ...
                positionGroup.push({
                    ...
                    votesAllowed: value.votesallowed
                });
            }
        });
        displayCandidate();
    ...
    function votesAllowed_1(element_id, candidate_id, candidate_name, candidate_party) {
        ...
        if (!id) {
            ...
            votes.push({
                ...
                positionName: positionGroup[targetPosition].positionName,
    ...
    $("#votingReceipt").on("show.bs.modal", function(event) {
        positionGroup.forEach(position => {
            $('#receipt').append(`
                <div id="${position.positionName.replace(/\s/g, '').toLowerCase()}">
        ...
        votes.forEach(vote => {
            $(`#${vote.positionName.replace(/\s/g, '').toLowerCase()}`).append(`
                <h6>${vote.candidateName} (${vote.candidateParty})</h6>
            `);
        })
