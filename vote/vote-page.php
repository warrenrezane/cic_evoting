...
<!-- Voting Receipt -->
<div class="modal fade" id="votingReceipt" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">

        <div class="modal-header bg-primary text-white">
            <h4 class="modal-title text-center">Voting Receipt</h4>
            <button type="button" class="close" onclick="votingReceiptModalClose()">&times;</button>
        </div>

            <div class="modal-body text-center" id="receipt"></div>

            <div class="modal-footer">
                <button type="submit" onclick="votingReceiptModalClose()" class="btn btn-default">Close</button>
            </div>
        </div>
    </div>
</div>
...
<div id="studentCode">
    ...
    <div class="row justify-content-center">
        <img src="../imgs/iclc_logo.png" width="100px" height="100px" id="logo" style="opacity:50%;">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
            <h1 class="text-center text-white">ICLC Voting System</h1>
            <div class="card student-code">
            <div class="d-flex flex-row">
            Voting Code <?php echo !$cannot_vote ? '<span class="text-warning">(Not Available)</span>' : '' ?></div>
            <input class="form-control" type="text" placeholder="Input Voting Code..." id="votingCodeInput" <?php echo !$cannot_vote ? 'disabled' : ''; ?>>
            <button class="btn btn-info btn-block mt-2" onclick="submitVotingCode()">Submit</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
   ...
   function disable_enable_prev_next() {
    ...
    function votesAllowed_1(element_id, candidate_id, candidate_name, candidate_party) {
        ...
        if(!id){
            ...
            votes.push({candidate_id:candidate_id,positionName:positionGroup[targetPosition].positionName,candidateName:candidate_name,candidateParty:candidate_party});
        } else {
    ...
    function votesAllowed_more(element_id, candidate_id, candidate_name, candidate_party) {
        const id = votes.find(vote => {
            if(vote.candidate_id == candidate_id && vote.positionName == positionGroup[targetPosition].positionName) {
                return true;
            }
    ...
    function checkUncheck(element_id, candidate_id, candidate_name, candidate_party) {
        if(parseInt(positionGroup[targetPosition].votesAllowed) == 1) {
            votesAllowed_1(element_id, candidate_id, candidate_name, candidate_party);
        } else {
            votesAllowed_more(element_id, candidate_id, candidate_name, candidate_party);
        }
    }
   ...
   function checkRadioButton(element_id) {
                        ...
                        <div class="d-flex flex-row">
                            <div class="candidate-img"><img src="${image}" width="40" height="40"></div>
                                ...
                                <div class="radio-wrapper">
                                    <div class="radio" id="check${candidate.id}" onclick="checkUncheck('check${candidate.id}',${candidate.id}, '${candidate.lastname.toUpperCase()}, ${candidate.firstname.toUpperCase()}', '${candidate.partyname.toLowerCase()}')">
    ...
    $("#votingReceipt").on("show.bs.modal", function (event) {
        let president = votes.find((vote) => vote.positionName === "President");
        president = president === undefined ? "----UNDERVOTE----" : president.candidateName + " (" + president.candidateParty.toUpperCase() + ")";

        let vicePresident = votes.find((vote) => vote.positionName === "Vice President");
        vicePresident = vicePresident === undefined ? "----UNDERVOTE----" : vicePresident.candidateName + " (" + vicePresident.candidateParty.toUpperCase() + ")";
        ...
        let senators = votes.filter((vote) => vote.positionName === "Senator");
        let temp = 3 - senators.length;

        senators.forEach((senator) => {
            $("#receipt").append(`
                <h6>${senator.candidateName} (${senator.candidateParty.toUpperCase()})</h6>
            `);
        })
    ...
    function votingReceiptModalClose() {
        if(confirm('Are you sure to close your voting receipt? You can only view this once.')) {
            voteStatusSend();
            votes = [];
            targetPosition = 0;
            positionGroup = [];
            ...
            $("#votingReceipt").modal("hide");
        }
    }

    $("#votingReceipt").on("hide.bs.modal", function (event) {
        $("#receipt").empty();
    });
</script>
</html>
